...
using Customer_Mangment.CQRS.Identity.Commands.GenerateTokens;
...
namespace Customer_Mangment.Controllers
{
    ...
    public sealed class AuthController(IDispatcher sender, IStringLocalizer<SharedResource> localizer, IIdentityServerTokenService identityServerToken, IIdentityService identityService) : ApiController(localizer)
    {
        ...
        public async Task<IActionResult> GenerateToken([FromBody] GenerateTokenQuery request, CancellationToken ct)
        {
            ...
            var result = await _sender.Send(new GenerateTokenComand(request.Email, request.Password, tenantId), ct);
            return result.Match(
        ...
        {
            var result = await _sender.Send(new RefreshTokenComand(request.RefreshToken, request.ExpiredAccessToken), ct);
            return result.Match(
               response => Ok(response),
               Problem);
        }
