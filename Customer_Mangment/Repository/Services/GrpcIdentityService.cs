...
public sealed class GrpcIdentityService(
    IIdentityGrpcService grpcClient,
    IStringLocalizer<SharedResource> localizer) : IIdentityService
{
    ...
    {
        var response = await grpcClient.ValidateUser(
        ...
        return new AppUserDto(response.UserId, response.Email, response.Roles, []);
    }

    public async Task<Model.Results.Result<AppUserDto>> GetUserByIdAsync(string userId)
    {
        var response = await grpcClient.GetUserById(
